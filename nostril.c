...
void usage()
{
    ...
    printf("  OPTIONS\n");
    printf("\n");
    printf("      --dm <hex pubkey>               make an encrypted dm to said "
           "pubkey. sets kind and tags.\n");
    printf("      --envelope                      wrap in [\"EVENT\",...] for "
           "easy relaying\n");
...
inline static int cursor_push_escaped_char(struct cursor* cur, char c)
{
    ...
    case '\\':
        return cursor_push_str(cur, "\\\\");
    case '\b':
        return cursor_push_str(cur, "\\b");
    case '\f':
        return cursor_push_str(cur, "\\f");
    case '\n':
        return cursor_push_str(cur, "\\n");
    case '\r':
        return cursor_push_str(cur, "\\r");
    case '\t':
        return cursor_push_str(cur, "\\t");
...
int event_commitment(struct nostr_event* ev, unsigned char* buf, int buflen)
{
    ...
    ok = cursor_push_str(&cur, "[0,\"") && cursor_push_str(&cur, pubkey) &&
         cursor_push_str(&cur, "\",") && cursor_push_str(&cur, timebuf) &&
         cursor_push_str(&cur, ",") && cursor_push_str(&cur, kindbuf) &&
         cursor_push_str(&cur, ",") && cursor_push_tags(&cur, ev) &&
         cursor_push_str(&cur, ",") && cursor_push_jsonstr(&cur, ev->content) &&
         cursor_push_str(&cur, "]");
...
static int make_sig(secp256k1_context* ctx, struct key* key, unsigned char* id,
                    unsigned char sig[64])
{
...
static int decode_key(secp256k1_context* ctx, const char* secstr,
                      struct key* key)
{
...
static int sign_event(secp256k1_context* ctx, struct key* key,
                      struct nostr_event* ev)
{
...
static int nostr_add_tag(struct nostr_event* ev, const char* t1,
                         const char* t2)
{
                             ...
                             unsigned char nostr_pubkey[32])
{
    ...
    if ((enclen = base64_encode((char*)buf, buflen, (const char*)encbuf,
                                enclen)) == -1) {
        fprintf(stderr,
                "make_encrypted_dm: base64 encode of encrypted fata failed\n");
        return 0;
    }
    cur.p += enclen;
    ...
    if ((enclen = base64_encode((char*)cur.p, cur.end - cur.p, (const char*)iv,
                                16)) == -1) {
